...
class Tndp:
    ...
    def add_route(self, r):
        self.routes.append(self.conct(r))
        self.route_nodes.append(set(r))
        self.route_arcs.append((r[0: int(len(r)/2)], r[int(len(r)/2):len(r)]))
        self.route_arc_flows.append({})
        self.route_max_flows.append([0])
        self.route_freq.append(1)
        self.output_freq.append(1)
        self.route_times.append(self.calc_path_time(r)/2)
    ...
    def build_transfer(self, i, j, pair):
        ...
        min_time = 1000000
        for node in common:
            auxr1 = self.build_path(i, node, self.routes[pair[0]])
            auxr2 = self.build_path(node, j, self.routes[pair[1]])
    ...
    def build_transfer_2(self, i, j, triple):
        ...
        min_time = 1000000
        for node in common1:
            auxr1 = self.build_path(i, node, self.routes[triple[0]])
    ...
    def compute_0_transfer_time(self, i, j, possible_routes, freq):
        ...
        for r in possible_routes:
            total_freq += freq[r]
        for r in possible_routes:
            pi = self.demand_matrix[i][j]*(freq[r]/total_freq)
            current_path = self.build_path(i,j,self.routes[r])
    ...
    def compute_1_transfer_time(self, i, j, possible_routes, freq):
        ...
        for key, val in trip_classes.items():
            ...
            for r in val:
                pi = self.demand_matrix[i][j]*freq[key]/total_freq*demand_factor
    ...
    def evaluate(self, routes, freq):
        ...
        for i in range(len(self.network)):
            for j in range(len(self.network)):
                if self.demand_matrix[i][j] == 0: continue
